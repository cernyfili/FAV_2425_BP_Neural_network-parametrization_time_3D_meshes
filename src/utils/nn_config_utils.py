...
def run_throuh_nn_at_decoder_time(inputs : torch.tensor, model : callable, decoder_time: TimeFrame) -> torch.tensor:
...
def _get_decoder_input_data(device, inputs, model):
    encoded_data = run_through_encoder(inputs, model.encoder)
...
def _loss_function_chamfer_better_random_dist(inputs, targets, model, loss_info):
    ...
    loss_chamfer_list = _compute_loss_chamfer_distance_with_time_tensor(decoder_data=decoder_data,
...
def _compute_loss_function_centers(inputs, targets, model, loss_info) -> list:
    ...
    for input_time_index in unique_time_elements:
        ...
        decoded_points = run_throuh_nn_at_decoder_time(inputs_at_time, model, decoder_time)
